...
which.min(student1)
```
What I want is to now drope (i.e. exclude) this lowest score from my mean() calculation.
```{r}
# This will return everything but the eight elements of the vector
student1[-8]
```
...
We could try using the na.rm=TRUE for mean but this is not a good approach because it is unfair.
```{r}
student2 <- c(100, NA, 90, 90, 90, 90, 97, 80)
...
which(is.na(x))
```
Now we have identified the NA elements, now we want to "mask" them. Replace them with zero?
...
mean(x)
```
Remember we should drop the lowest score now... 
...
Take the snipper and turn it into a function
...
```{r}
#' Calculate the average score for a vector of students scores dropping the lowest score. M issing values will be treated as zeros.
#'
...
Now finally we can use our function on our "real" whole class data from this CSV format file:
...
```{r}
ave.scores <- apply(gradebook, 2, median, na.rm=TRUE)
ave.scores
which.min(ave.scores)
```
